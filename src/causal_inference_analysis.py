...
basic_model = ols('Rides ~ Group + Event + Did', data=df).fit()
print("Basic DiD Model Results:")
print(basic_model.summary())

# Step 3: Advanced DiD Model with Controls
advanced_model = ols('''Rides ~ Group + Event + Did +
...
for group in [0, 1]:
    ...
    plt.scatter(group_data['Date'], group_data['Rides'],
    ...
    for event in [0, 1]:
        ...
        z = np.polyfit(event_data['Date'].astype(int) / 10 ** 9, event_data['Rides'], 1)
        p = np.poly1d(z)
...
plt.title("DiD Visualization: Taxi Rides Over Time")
plt.xlabel("Date")
plt.ylabel("Rides")
plt.legend()
plt.show()
...
control_trend = sm.OLS.from_formula('Rides ~ Days', data=pre_uber[pre_uber['Group'] == 0]).fit()
treatment_trend = sm.OLS.from_formula('Rides ~ Days', data=pre_uber[pre_uber['Group'] == 1]).fit()
...
placebo_model = ols('''Rides ~ Group + Placebo_Event + Placebo_Did +
                       Population + Employment_Rate + Gas_Price + Mean_Temp +
                       Month_2 + Month_3 + Month_4 + Month_5 + Month_6 + 
                       Month_7 + Month_8 + Month_9 + Month_10 + Month_11 + Month_12''',
...
heterogeneous_model = ols('''Rides ~ Group + Event + Did + Did:High_Population +
                           Population + Employment_Rate + Gas_Price + Mean_Temp +
                           Month_2 + Month_3 + Month_4 + Month_5 + Month_6 + 
                           Month_7 + Month_8 + Month_9 + Month_10 + Month_11 + Month_12''',
                          data=df).fit()
...
plt.title('Heterogeneous Effects of Uber on Taxi Rides')
...
long_term_model = ols('''Rides ~ Group + Event + Did + Years_Since_Uber +
                           Population + Employment_Rate + Gas_Price + Mean_Temp +
                           Month_2 + Month_3 + Month_4 + Month_5 + Month_6 + 
                           Month_7 + Month_8 + Month_9 + Month_10 + Month_11 + Month_12''',
                      data=df).fit()
...
plt.title('Estimated Long-term Effects of Uber on Taxi Rides')
